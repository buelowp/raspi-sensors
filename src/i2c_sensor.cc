...
namespace sensor {
    ...
    void i2c_sensor::readRegisterArray(uint16_t address, uint8_t *array, uint8_t size)
    {
         ...
         if(write(getBus(), &address, 1) != 1)
             goto error;
         if(read(getBus(), array, size) != size)
             goto error;
     ...
     */
    void i2c_sensor::writeRegisterArray(uint8_t address, uint8_t* value, uint8_t size) {
        uint8_t buf[size + 1] = {0}; // buffer for write.
        uint8_t bufsize = size + 1;
        // Open the bus
        openBus();
        ...
        // Send the register and the data
        if(write(getBus(), buf, bufsize) != bufsize)
            goto error;
